    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (!isNaN(date.getTime())) {
            ...
            if (times.length > 100000) {
                console.log("Предупреждение: ограничение обработки временных меток для производительности");
                break;
            }
        }
    }
...
function generateStatisticsSimple(rows) {
    ...
    for (let i = 0; i < Math.min(rows.length, 100000); i++) { // Ограничиваем для больших файлов
        const row = rows[i];
...
async function saveToMultipleFormats(rows, baseName) {
    ...
    if (rows.length <= 500000) {
        ...
        const writeStream = require('fs').createWriteStream(csvFile, { encoding: 'utf8' });
        writeStream.write('startTime,endTime,probability,latitude,longitude,source\n');
        ...
        for (let i = 0; i < rows.length; i += batchSize) {
            ...
            if (i % 100000 === 0) {
                console.log(`  CSV: записано ${Math.min(i + batchSize, rows.length)} из ${rows.length} строк`);
            }
        }
    ...
    if (rows.length <= 100000) {
        ...
        const writeStream = require('fs').createWriteStream(jsonFile, { encoding: 'utf8' });
        writeStream.write('[\n');
...
async function main() {
    try {
        ...
        console.log(`📊 Excel: ${savedFiles.excel.path} (${savedFiles.excel.rows} строк из ${savedFiles.excel.totalRows})`);
